  ...
  return (
    ...
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        disabled={isPending}
        className="flex items-center gap-[5px] text-[14px] font-medium text-[#1a1a1a] hover:text-[#555] transition-colors duration-150 disabled:opacity-50"
      >
        {lang.toUpperCase()}
        <span
          className="text-[10px] text-[#aaa] transition-transform duration-150"
      ...
      {open && (
        <div className="absolute right-0 top-[calc(100%+6px)] bg-white border border-[#ebebeb] rounded-[7px] shadow-sm overflow-hidden z-50">
          <button
            ...
            className="block w-full px-4 py-[10px] text-[14px] text-[#555] hover:text-[#1a1a1a] hover:bg-[#f7f7f7] transition-colors duration-100 whitespace-nowrap"
          >
            {other.toUpperCase()}
          </button>
        </div>
