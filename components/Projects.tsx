...
import { PiArrowUpRight, PiCaretDown, PiMedalFill } from 'react-icons/pi'
...
const card: Variants = {
  ...
  exit: { opacity: 0, y: -12, scale: 0.97, transition: { duration: 0.25, ease: 'easeIn' } },
}
...
export function Projects({ lang }: { lang: Lang }) {
  ...
  const visibleProjects = expanded ? projectsData : projectsData.slice(0, VISIBLE_COUNT)
      ...
      {hasMore && (
        <div className="flex justify-center mt-6">
          <button
            ...
            className="group inline-flex items-center gap-[7px] rounded-full border border-[#e4e1d8] bg-white py-[9px] pl-[16px] pr-[14px] text-[12px] font-medium text-[#666] shadow-[0_1px_2px_rgba(0,0,0,0.04)] transition-all duration-200 hover:border-[#1a1a1a] hover:text-[#1a1a1a] hover:shadow-[0_2px_8px_rgba(0,0,0,0.08)]"
          >
            {expanded ? t.showLessProjects : t.showMoreProjects}
            <PiCaretDown
              className={`text-[13px] transition-transform duration-300 ${expanded ? 'rotate-180' : ''}`}
            />
          </button>
        </div>
      )}
