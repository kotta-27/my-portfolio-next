...
export function Nav({ lang }: { lang: Lang }) {
  const t = ui[lang]
  return (
    <nav className="sticky top-0 z-50 bg-[#edeae3] border-b border-black/[.07] px-10 flex justify-between items-center h-[58px]">
      <a
        href="?"
      ...
      <div className="flex gap-[26px] items-center">
        {t.nav.map((label) => (
          <a
            ...
            className="text-[12px] text-[#555] no-underline hover:text-[#1a1a1a] transition-colors duration-150"
          >
            {label}
          </a>
        ))}
